...
function findExpression(node, scope) {
  switch (node.type) {
    ...
    case NodeTypes.BinaryExpression:      
      {
        const left = (match(node.left, scope));
        const right = (match(node.right, scope));
        const canCalLeft = utils.isCanCalType(left);
        const canCalRight = utils.isCanCalType(right);
        if (utils.isCanCalType(left, right)) {
          return runner.doCal(left, right, node.operator);
        }
        return {
          ...node,
          left: canCalLeft ? runner.createNodeByType(left) : node.left,
          right: canCalRight ? runner.createNodeByType(right) : node.right,
        };
      }
    ...
    case NodeTypes.CallExpression:
      {
        ...
        // 缺省输出
        if (!utils.isCalleeNode(calleeNode)) {
          return {
            ...node,
